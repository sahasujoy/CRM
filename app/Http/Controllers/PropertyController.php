...
// }
    ...
    public function bnfDetailsView()
    {
        // $flat = Flat::where('building_id', 1)->where('sell_status', 'Unsold')->count();
        // dd($flat);
        return view('property.bnf_details');
    }

    public function fbyb($id)
    {
        $building = Building::find($id);
        // $flats = Flat::where('building_id', $id)->whereHas('customer')->get(); //call those flats who have registration with a customer
        $flats = Flat::where('building_id', $id)->get(); //call those flats who have registration with a customer
